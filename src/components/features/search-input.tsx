	...
	return (
		<div ref={ref} className={cn('relative w-full', className)}>
			<div
				ref={ref}
				className={
					'relative flex h-10 w-96 items-center gap-3 rounded-md border bg-slate-50 p-2 px-3 pl-10 transition-all duration-200 hover:border-slate-300 max-md:w-full dark:bg-slate-900 dark:hover:border-slate-700'
				}
			>
				{isLoading ? (
				...
				<input
					...
					type='text'
					placeholder='Поиск...'
					className='placeholder:text-muted-foreground w-full border-0 bg-transparent px-0 text-sm outline-none focus:ring-0'
					autoComplete='off'
			...
			<AnimatePresence>
				{focused && (
					<motion.div
						...
						className='scrollbar-thin dark:scrollbar-thumb-slate-800 dark:scrollbar-track-slate-950 absolute top-12 left-0 max-h-[50dvh] w-full overflow-y-auto rounded-lg border bg-slate-50 p-2 dark:bg-slate-900'
					>
						{isLoading ? (
							new Array(3)
								.fill(0)
